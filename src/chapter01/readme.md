...
* [虚拟照相机模型](#虚拟照相机模型)
* [应用程序编程接口](#应用程序编程接口)
* [图形绘制系统的体系结构](#图形绘制系统的体系结构)
* [可编程流水线](#可编程流水线)
* [性能特征](#性能特征)
* [OpenGL版本和WebGL](#OpenGL版本和WebGL)
## 1.1 计算机图形学的应用
- 信息的展示【可视化技术】
- 设计【工程建筑设计】
- 仿真和动画【VR】
...
### 1.7.1 显示器处理器
    制作专用图形系统的努力主要是为了把通用计算机从不间断刷新显示器的任务中解放出来。
...
### 1.7.4 顶点处理
    在绘制流水线的第一个模块中，对各个订单的处理是彼此独立的。这个模块的主要功能是执行坐标变换。这个模块也计算每个顶点的颜色值并改变每个顶点的其他属性。

    坐标系的每一次变换都可以用一个矩阵来表示。坐标系的多次变换可以表示为矩阵的相乘或者级联，于是多个矩阵通过相乘就合并为一个矩阵。

### 1.7.5 裁剪和图元组装
    标准图形绘制流水线中第二个基本模块的任务是裁剪和组装图元。因为成像系统不可能一次对整个场景成像，基于这个限制必须进行裁剪。虚拟照相机的视域也是有限的，这是通过裁剪体来体现的。
    
    裁剪必须针对逐个图元进行，而不是针对逐个顶点。

### 1.7.6 光栅化
    由裁剪模块得到的图元仍然是用顶点表示的，为了生成帧缓存中的像素还必须进一步处理。

### 1.7.7 片元处理
    绘制流水线的最后一个模块利用光栅化模块生成的片元来更新帧缓存中的像素。

## 1.8 可编程流水线
    在可编程流水线中，顶点着色器可以在顶点经过流水线时修改每个顶点的位置或颜色。这样，就可以实现许多光线-材质模型或者创建新型的投影。片元着色器允许我们以许多新的方式来使用纹理，也可以使用片元着色器实现流水线中的其他处理部分，例如实现基于每个片元的光照而不是基于每个顶点的光照。

## 1.9 性能特征
    由于处理顶点和处理片元所执行的操作非常类似，现代GPU已演变成统一着色引擎，它能够同时完成顶点着色和片元着色。

## 1.10 OpenGL版本和WebGL
    OpenGL的前身是SCI公司为其革命性的VLSI绘制流水线图形工作站开发的IRIS GL API。

    OpenGL:
    Version 1.0 致力于立即绘制模式
    Version 2.0 引入OpenGL着色语言，延迟绘制模式
    Version 3.0 向后兼容
        -Version 3.1 不支持立即绘制模式
