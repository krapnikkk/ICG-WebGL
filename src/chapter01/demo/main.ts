...
class DrawPixel {
    ...
    /**
     *
     * @param {Array} points 需要绘制的坐标数组
     */
    writePixel(points:vec2[]) {
      // 清空坐标
      this._points = [];
     ...
     */
    _initGl() {
      const canvas:HTMLCanvasElement = document.querySelector('#gl') as HTMLCanvasElement;
      const gl = GLHelper.setupWebGL(canvas)!;
    ...
    _render() {
      ...
      gl.bufferData(gl.ARRAY_BUFFER, GLHelper.pointsToBuffer(this._points), gl.STATIC_DRAW);
  ...
  drawPixel.drawCircle([0.0, -0.4], 0.1, 20);